...
2. **Activar el CAPTCHA nativo de Supabase Auth** (F-012).
   Dashboard → Authentication → Settings → _Enable Captcha protection_.
   El login server-side impide saltarse Turnstile **a través de la aplicación**,
   pero el endpoint `/auth/v1/token` de Supabase es público por diseño: sin el
   CAPTCHA nativo, la fuerza bruta directa sigue siendo posible y solo la limita
   el rate limit propio de Supabase.
3. **Deshabilitar el registro público** en Supabase Auth (defensa en profundidad
   sobre F-001). El fix del trigger ya impide la escalada aunque el signup esté
   abierto; deshabilitarlo elimina también el alta de cuentas basura.
4. **Rotar `SUPABASE_JWT_SECRET`** una vez confirmado que el proyecto usa llaves
   asimétricas y `ALLOW_LEGACY_HS256` queda apagado (F-030).
5. **Configurar un monitor HTTP contra `/health`** en Better Stack (F-011). El
