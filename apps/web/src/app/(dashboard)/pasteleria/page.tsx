...
export const metadata: Metadata = {
  title: 'Pastelería — Dorado Lounge',
};
...
export default async function PasteleriaPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Pastelería</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Producción de pastelería · Pedidos especiales desde Amex
        </p>
      </div>
      ...
      {!turnoActivo && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 px-4 py-3">
          <p className="text-sm text-amber-800 dark:text-amber-300 font-medium">
            Sin turno activo — contacta al administrador para abrir turno.
          </p>
        </div>
      )}
      ...
      {pedidosEspeciales.length > 0 && (
        <div className="rounded-lg border border-violet-200 bg-violet-50 dark:bg-violet-950/20 dark:border-violet-800 p-4 space-y-3">
          <p className="text-sm font-semibold text-violet-800 dark:text-violet-300">
            {pedidosEspeciales.length} pedido{pedidosEspeciales.length > 1 ? 's' : ''} especial
            {pedidosEspeciales.length > 1 ? 'es' : ''} pendiente
            {pedidosEspeciales.length > 1 ? 's' : ''} desde Amex
          </p>
          <ul className="space-y-1.5">
            {pedidosEspeciales.map((p) => (
              ...
              >
                <span className="font-medium">{p.numeroMesa ?? 'Sin mesa'}</span>
                <div className="flex items-center gap-3">
                  ...
                  >
                    {p.estado === 'creado' ? 'Nuevo' : 'En preparación'}
                  </span>
                </div>
              </li>
            ))}
