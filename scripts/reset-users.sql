...
--
-- Resultado: 10 usuarios con contraseña Admin123 (cambiar en producción).
-- El DELETE en auth.users elimina también public.users por CASCADE (FK).
-- =============================================================================
...
BEGIN
  ...
  -- CASCADE elimina automáticamente las filas de public.users (FK ON DELETE CASCADE)
  DELETE FROM auth.users;
  RAISE NOTICE '✓ Usuarios anteriores eliminados';

  -- ── 2. Garantizar que el tenant existe ───────────────────────────────────
  ...
  --   raw_user_meta_data  → el trigger lo copia a app_metadata (por si acaso)
  --   raw_app_meta_data   → lo que realmente leen las RLS y el JWT
  ...
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════';
  RAISE NOTICE 'CREDENCIALES DE ACCESO  (contraseña: Admin123)';
  RAISE NOTICE '────────────────────────────────────────────────────';
